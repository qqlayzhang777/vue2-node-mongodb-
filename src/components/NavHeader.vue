<template>
  <header class="header">
    ...
    <div class="navbar">
      ...
      <div class="navbar-right-container" style="display: flex;">
        <div class="navbar-menu-container">
          ...
          <span class="navbar-link" v-if="!loginSuc">{{userName}}</span>
          <a href="javascript:void(0)" class="navbar-link" @click="LoginEvent" v-if="loginSuc">Login</a>
          <a href="javascript:void(0)" class="navbar-link" v-if="!loginSuc" @click="Logout">Logout</a>
          <div class="navbar-cart-container">
            <span class="navbar-cart-count">12</span>
            <router-link class="navbar-link navbar-cart-link" :to="userCart">
              <svg class="navbar-cart-logo"  @click="clickCart">
    ...
    <div class="md-modal modal-msg md-modal-transition" :class="[{'md-show':loginShow}]">
      <div class="md-modal-inner">
        <div class="md-top">
          <div class="md-title">Login in</div>
          <button class="md-close" @click="loginShow=false;logErrShow=false">Close</button>
        </div>
        <div class="md-content">
          <div class="confirm-tips">
            <div class="error-wrap">
              <span class="error" :class="[{'error-show':logErrShow}]">用户名或者密码错误</span>
            </div>
            <ul>
              <li class="regi_form_input">
                <i class="icon IconPeople"></i>
                <input type="text" tabindex="1" name="loginname" class="regi_login_input regi_login_input_left" placeholder="User Name" data-type="loginname" v-model="usersName" @keydown.enter="checkLogin" v-focus>
              </li>
              <li class="regi_form_input noMargin">
                <i class="icon IconPwd"></i>
                <input type="password" name="password" placeholder="Password" tabindex="2" class="regi_login_input regi_login_input_left login-input-no input_text" v-model="userPwd" @keydown.enter="checkLogin">
              </li>
            </ul>
          </div>
          <div class="login-wrap">
            <a href="javascript:;" class="btn-login" @click="checkLogin">登  录</a>
          </div>
        </div>
      </div>
    </div>
    <div class="md-overlay" v-if="loginShow" @click="loginShow=false;logErrShow=false">
    </div>
    <Modal :mdAll="mdAll" @close="closeModal" class="md-modal-all">
      <p slot="content" class="md-modal-text">请 先 登 录</p>
      <a href="javascript:;" slot="sure" @click="mdAll=false;loginShow=true" class="btn btn--m">登 录</a>
    </Modal> 
  </header>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      checkLog () {
        axios.get('/users/checkLogin').then(response=>{
          ...
          if(res.status == '0'){
            ...
            this.$store.commit('userNameMuta',res.result)
            this.loginSuc = false
            this.userCart = 'cart'
          }
        })
      ...
      checkLogin () {
        if(!this.usersName || !this.userPwd){
          this.logErrShow = true
          return
        }
        axios.post('/users/login',{
          ...
          if(res.status == '0'){
            if(res.msg){
              ...
              // this.$store.commit('userNameMuta',res.result.userName)
              this.loginSuc = false
              this.loginShow = false
              this.logErrShow = false
              this.userCart = 'cart'
              this.$emit("init")
              this.$emit("addressGet")
              this.$emit("orderGet")
              console.log(res.result.userName)
          }else{
            this.logErrShow = true
            console.log(res.msg + ' ' + res.think)
          }
        })
      }, 
      //退出登录
