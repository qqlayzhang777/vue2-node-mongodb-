<template>
	<div>
		<nav-header @orderGet="orderGet"></nav-header>
		<nav-bread>
			<span slot="Cart">OrderList</span>
		</nav-bread>
		<div>
		    ...
		    <div class="checkout-order">
		        ...
		        <div class="price-count">
		          <ul>
		            ...
		            <li class="order-total-price">
		              ...
		              <span>{{totalAllPrice | currency('¥ ')}}</span>
		            </li>
		          </ul>
		        </div>
		      </div>
		      ...
		      <div class="order-foot-wrap">
		        <div class="prev-btn-wrap">
		          <router-link class="btn btn--m" :to="previous">Previous</router-link>
		        </div>
		        <div class="next-btn-wrap">
		          <!-- <router-link class="btn btn--m btn--red" :to="orderCom" @click="">Proceed to payment</router-link> -->
		          <button class="btn btn--m btn--red" @click="saveOrder">Proceed to payment</button>
		        </div>
		      </div>
		    </div>
		  </div>
...
<script>
	...
	export default {
		...
		computed: {
			...
			totalAllPrice () {
				return (this.totalPrice * this.discount / 100) + this.shipping + this.tax
			}
		},
		methods: {
			//请求addressList列表
			...
			saveOrder () {
				var param = {
					...
					tax:this.tax,
					orderTotal:this.totalAllPrice,
					goodsList:this.orderList,
					addressId: this.$route.query.addressId
				}
				console.log(param.addressId)
				axios.post('users/createOrder',{params:param}).then(response=>{
					var res = response.data
					if(res.status == 0){
						this.$router.push({
