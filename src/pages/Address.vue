<template>
	<div>
		<nav-header @addressGet="addressGet"></nav-header>
		<nav-bread>
			<span slot="Cart">Address</span>
		</nav-bread>
		  ...
		  <div class="container">
		    <div class="checkout-addr">
		      ...
		      <div class="addr-list-wrap">
		        <div class="addr-list">
		          <ul>
		            <li v-for="(item,index) in addressMore" :class="[{check:checked==index}]" @click="checked=index;selectAddId=item.addressId">
		              <dl>
		                <dt>{{item.userName}}</dt>
		              ...
		              <div class="addr-opration addr-del">
		                <a href="javascript:;" class="addr-del-btn" @click="delAdd(index)">
		                  <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
		                </a>
		              </div>
		              <div class="addr-opration addr-set-default">
		                <a href="javascript:;" class="addr-set-default-btn" v-if="!item.isDefault" @click.stop="setDefault(item,index)"><i>Set default</i></a>
		              </div>
		              <div :class="[{'addr-opration addr-default':item.isDefault}]" v-if="item.isDefault">Default address</div>
		            </li>
		            <li class="addr-new">
		        ...
		        <div class="shipping-addr-more">
		          <a class="addr-more-btn up-down-btn" href="javascript:;" @click="moreAddress">
		            {{more}}
		            <i class="i-up-down" :class="[{mol:moreAdd},{mos:!moreAdd}]">
		      ...
		      <div class="next-btn-wrap">
		        <router-link class="btn btn--m btn--red" :to="{path:toOrder,query:{'addressId':selectAddId}}">Next</router-link>
		      </div>
		    </div>
		  </div>
		</div>
...
<style>
	...
	.mos{
		margin-top: 3px;
		animation:less .5s ease-in both;
	}
	.mol{
		margin-top: 5px;
		animation:more .5s ease-in both;
	}
</style>
...
<script>
	...
	export default {
		...
		methods: {
			...
			addressGet () {
				axios.get('users/address').then(response=>{
					...
					if(res.status == 0){
						...
						if(!this.moreAdd){
							this.addressMore = this.addressList.slice(0, 3)
						}
						this.toOrder = 'order'
					}else{
						this.moreAdd = false
						this.addressMore = this.addressList = []
						this.toOrder = ''
			...
			//保存当前地址列表到服务器
			saveAddress () {
				axios.post('users/address',{addressList:this.addressList}).then(response=>{
					var res = response.data
					if(res.status == 0){
